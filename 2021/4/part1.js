...
function calcFinalScore(board, winningNum) {
  let sum = 0;
  for (const num in board) {
    if (Object.hasOwnProperty.call(board, num)) {
      const numInfo = board[num];
      if (!numInfo.called) {
        sum += +num;
      }
    }
